...
const ItemInformation = ({handleShowAddItemForm, showItemInformation}) => {
    return (
        <div className={`w-shopping_list min-h-screen flex flex-col py-4 px-6 gap-4 fixed right-0 top-0 bottom-0 phone:${showItemInformation ? 'hidden' : 'w-content'} phone:px-3`}>
            <div className='flex flex-row justify-start items-center gap-2'>
                <Image src={BackIcon} width={20} height={20} />
                <p className='text-sm font-bold text-yellow font-quicksand phone:text-xs'>Back</p>
            </div>
            <div className='flex flex-col justify-center items-start gap-4'>
                <Image src={AguacateImg} width={250} height={250} className='w-full rounded-md'/>
